            ...
            <li key={link.href}>
              <Link
                ...
                className={`px-2.5 py-2 rounded-lg text-xs font-medium transition-all hover:bg-green-50 hover:text-green-700 ${
                  pathname === link.href ? "text-green-700 bg-green-50" : "text-gray-700"
                }`}
              >
                {link.label}
              </Link>
            ...
            {navLinks.map((link) => (
              <li key={link.href}>
                <Link
                  ...
                  className={`block px-4 py-3 font-medium rounded-lg hover:bg-green-50 hover:text-green-700 transition-colors ${
                    pathname === link.href ? "text-green-700 bg-green-50" : "text-gray-700"
                  }`}
                >
                  {link.label}
